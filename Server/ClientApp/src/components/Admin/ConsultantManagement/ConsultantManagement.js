...
import ConsultantManagementTable from './ConsultantManagementTable';
...
class ConsultantManagement extends Component {
  ...
  async componentWillMount() {
    ...
    handleChange = (n, e, fieldName) => {
      ...
      const data = this.state.Consultants;
    ...
    handleIsApproved = (n,e,val) => {
       if (e.target.checked === true)
       {
        const data = this.state.Consultants;
        const index = data.indexOf(n);
        fetch('api/Consultants/approveConsultant', {
          method: 'POST',
      ...
      else 
      {
        const data = this.state.Consultants;
        const index = data.indexOf(n);
        fetch('api/Consultants/disapproveConsultant', {
          method: 'POST',
    ...
    handleDeleteConsultant = (n, e) => {
      if (n !== '') {
        fetch(`api/auth/removeConsultant?Id=${n}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('id_token')}` },
        })
          .then((res) => {
            if (res.status === 200) {
              this.setState({ open: false });
              alert('Removed Consultant Successfully');
            }
          })
          .catch(() => {
